...
urlpatterns = patterns('',
    ...
    url(r'^pie_chart/(\w{2})/regions/([\w ,-\\.]+)/stat_type/(\w+)/from_year/(\d{4})/to_year/(\d{4})/data', pie_chart_data),
    url(r'^pie_chart/(\w{2})/regions/([\w ,-\\.]+)/stat_type/(\w+)/from_year/(\d{4})/to_year/(\d{4})', pie_chart_url),
    url(r'^pie_chart/(\w{2})/', pie_chart),

    url(r'^sankey_chart/(\w{2})/regions/([\w ,-\\.]+)/stat_type/(\w+)/from_year/(\d{4})/to_year/(\d{4})/data', sankey_chart_data),
    url(r'^sankey_chart/(\w{2})/regions/([\w ,-\\.]+)/stat_type/(\w+)/from_year/(\d{4})/to_year/(\d{4})', sankey_chart_url),
    url(r'^sankey_chart/(\w{2})/', sankey_chart),
